...
#
# Fixes:
#   1. StartupWMClass matches the browser's --class (browseros for prod, browseros-dev for dev)
...
PYEOF
  ...
  # Local icons (orange 🔧 badge overlay)
  _create_local_badge() {
    local size="$1" badge="$2" fsize="$3"
    local src="${ICON_DIR}/${size}x${size}/apps/browseros.png"
    local dst="${ICON_DIR}/${size}x${size}/apps/browseros-local.png"
    if [[ ! -f "${src}" ]]; then
      echo "⚠ Missing ${src} — skip ${size} local icon"
      return
    fi
    python3 - <<PYEOF
from PIL import Image, ImageDraw, ImageFont
...
cy = img.size[1] - badge//2 - 4
r = badge//2
draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=(249, 115, 22, 240))
try:
    font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", fsize)
except Exception:
    font = ImageFont.load_default(size=fsize)
...
tw, th = bbox[2]-bbox[0], bbox[3]-bbox[1]
draw.text((cx-tw//2, cy-th//2-2), "L", fill=(255,255,255,255), font=font)
Image.alpha_composite(img, overlay).save(dst, "PNG")
print(f"✔ Created {dst}")
PYEOF
  }

  _create_local_badge 256 64 28
  _create_local_badge 128 32 14
...
# --- Desktop entries ---
# KEY FIX: Different StartupWMClass for prod vs dev
#
# Linux packaging uses StartupWMClass=browseros for BrowserOS.
# Keep the local desktop entry aligned with the real launcher class so
# GNOME associates the running prod window with the BrowserOS taskbar icon.
# Dev still launches with --class=browseros-dev for strict separation.
#
install_desktop_entries() {
  mkdir -p "${APP_DIR}"

  # ── Prod entry ──
  # Uses a tiny bash wrapper to add --no-sandbox (required on Linux) + port config.
  # On Wayland, GNOME matches the window via StartupWMClass=browseros
  # which matches the --class=browseros flag passed to the browser.
  # The wrapper uses exec so the AppImage replaces bash — GNOME can trace PID.
...
Exec=bash -c 'exec ${REPO_ROOT}/scripts/launch/launch-browseros-prod.sh'
Icon=browseros
Terminal=false
Type=Application
...
EOF
  ...
  echo "✔ Installed browseros.desktop (StartupWMClass=browseros)"
...
EOF
  ...
  echo "✔ Installed browseros-dev.desktop (StartupWMClass=browseros-dev, unique from prod)"

  # ── Local entry — runs extracted AppImage with local-built extension ──
  cat > "${APP_DIR}/browseros-local.desktop" <<EOF
[Desktop Entry]
Version=1.0
Name=BrowserOS (Local Build)
GenericName=Web Browser
Comment=BrowserOS built from local source — server + extension from latest code
Exec=bash ${HOME_DIR}/Downloads/alta/launch-browseros-local.sh
Icon=browseros-local
Terminal=false
Type=Application
Categories=Development;Network;WebBrowser;
StartupNotify=true
StartupWMClass=browseros-local
# Auto-generated by setup-desktop-entries.sh — do not edit manually
EOF
  chmod +x "${APP_DIR}/browseros-local.desktop"
  echo "✔ Installed browseros-local.desktop (StartupWMClass=browseros-local, unique from prod/dev)"
