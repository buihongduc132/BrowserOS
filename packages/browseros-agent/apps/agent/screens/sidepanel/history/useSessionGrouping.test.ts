...
import type { AssistantSession } from './useSessionGrouping'
import { groupSessions } from './useSessionGrouping'

// Helper to create test sessions
function makeSession(
  overrides: Partial<AssistantSession> & { id: string },
): AssistantSession {
  return {
    title: 'Test session',
    workspaces: [],
...
describe('groupSessions', () => {
  ...
  it('groups sessions by workspace ID', () => {
    const sessions = [
      makeSession({
        id: '1',
        workspaces: [{ id: 'ws-1', name: 'frontend', path: '/frontend' }],
      }),
      makeSession({
        id: '2',
        workspaces: [{ id: 'ws-2', name: 'backend', path: '/backend' }],
      }),
      makeSession({
        id: '3',
        workspaces: [{ id: 'ws-1', name: 'frontend', path: '/frontend' }],
  ...
  it('multi-tag session appears in multiple tag groups', () => {
    const sessions = [makeSession({ id: '1', tags: ['bug', 'urgent'] })]
  ...
  it('untagged sessions go to "Untagged" group', () => {
    const sessions = [makeSession({ id: '1', tags: [] })]
